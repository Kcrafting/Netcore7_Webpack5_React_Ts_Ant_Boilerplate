...
const useResizeTableCol = (wrapperWidth: number | undefined, tableRef: any, columns: ColumnType<any>[]) => {
  ...
  const handleResize = useMemoizedFn((index: number) => (e: any, { size }: any) => {
    ...
    if (tableRef.current) {
      const widthList = [
        ...
        if(typeof (th as unknown as HTMLElement).getBoundingClientRect == 'function'){
          return (th as unknown as HTMLElement).getBoundingClientRect().width;
        }
        return 0;
      });
      setTableColumns((col) => {
  ...
  useEffect(() => {
    if (isInit) {
      setTableColumns(
        tableShowColumns.map((col:any, index:any) => ({
          ...col,
          onHeaderCell:
