        ...
        public IEnumerable<Menu> Post()
        {
            return new List<Menu>() {
                new Menu() {label="入库",key="sub1",icon="MailOutlined", children = new List<Menu>{
                    new Menu() { label="产品入库导入",key="api/cprkdr",icon="" },
                    new Menu() { label="配件入库导入",key="api/pjrkdr",icon="" },
                    new Menu() { label="其他",key="sub2",icon="",children = new List<Menu>(){
                        new Menu() { label="入库导入设置",key="api/rkdrsz",icon="" }
                    }}
                }},
                new Menu() {label="出库",key="sub3",icon="", children = new List<Menu>{
                    new Menu() { label="产品出库导入",key="api/cpckdr",icon="" },
                    new Menu() { label="配件出库导入",key="api/pjckdr",icon="" },
                    new Menu() { label="其他",key="sub4",icon="",children = new List<Menu>(){
                        new Menu() { label="出库导入设置",key="api/ckdrsz",icon="" }
                    }}
                }},
                new Menu() {label="基础资料",key="sub5",icon="", children = new List<Menu>{
                    new Menu() { label="基础资料导入",key="api/jczldr",icon="" },
                    new Menu() { label="其他信息同步",key="api/qtxxtb",icon="" },
                }}
            };
        }
    }
